...
use nalgebra::{Dim, constraint::{DimEq, AreMultipliable, ShapeConstraint}};
use crate::filter::filter_traits::Filter;
use ndarray::linalg::Dot;
use std::ops::{AddAssign, Sub};
use crate::types::{Covariances, QuadraticForm, Matrix2, States2, TransposeView, States3};
...
/// on four matrices, which represent the operational parameters necessary for this algorithm.
...
pub struct KalmanFilter<T, S,M> where
    T: Scalar+Lapack,
    S: Dim,
    M: Dim
{
    transition_matrix: Matrix2<T, S, S>,
    observation_matrix: Matrix2<T, M, S>,
    transition_covariance: Matrix2<T, S, S>,
    observation_covariance: Matrix2<T, M, M>
}

/// Implementation of filtering methods for Kalman filter
impl<T, S, M> Filter<T, S, M> for KalmanFilter<T, S, M> where
    T: Scalar + Lapack,
    S: Dim,
    M: Dim

{
    ...
    type Predict = (States2<T, S>, Covariances<T,S>);
    type Update = ();

    fn predict<N>(&self, states: &States2<T, N>, covariances: &Covariances<T, N>) -> Self::Predict where
        N: Dim,
        ShapeConstraint: AreMultipliable<S, S, N, N> + AreMultipliable<N, N, S, S> + DimEq<S, N> + DimEq<N, S>
    {
        let states_prediction = self.transition_matrix.dot(states);
        let mut covariances_intermediate = covariances.quadratic_form(&self.transition_matrix, &self.transition_matrix);
        covariances_intermediate.add_assign(&self.transition_covariance);
        (states_prediction, covariances_intermediate)
    }
    ...
    {
        let expected_measurements = self.observation_matrix.dot(states);
        let innovations_negated = expected_measurements.pairwise_difference(&measurements);
        let l_matrices = covariances.dot(&self.observation_matrix.t_view());
        let mut u_matrices = self.observation_matrix.dot(&l_matrices);
        u_matrices.add_assign(&self.observation_covariance);
        let u_matrices_inv = u_matrices.invc().unwrap();
        let kalman_gains = l_matrices.dot(&u_matrices_inv);
        let innovation_updates = kalman_gains.dot(&innovations_negated);
        let updated_states = <&States2<T, N> as Sub<&States<T, N, ndarray::Ix3, OwnedRepr<T>>>>::sub(states, &innovation_updates);
        let covariance_update_matrices = kalman_gains.dot(&l_matrices.t_view());
        let updated_covariances = covariances.sub(&covariance_update_matrices);
